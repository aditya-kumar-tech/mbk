(function () {
    console.log("ðŸš€ Universal Loader v8.1 - Gold & Silver Auto Update with Retry & Graphs");
    ...
    for (const k in map) {
        const r = map[k].range;
        if (Array.isArray(r) && r.includes(n)) {  // âœ… range list à¤®à¥‡à¤‚ exact match
            return { id: map[k].id, off: r.indexOf(n) };  // âœ… list position = offset
        }
    }
    return null;
}
    ...
    function renderGold(rows){
        ...
        function updateGramTable(id,val){ 
            ...
            if(el){ 
                ...
                [1,5,10,50,100].forEach(g=>{ 
                    html+=`<tr style="border-bottom:1px solid #eee;"><td style="padding:6px;">${g}g</td><td style="text-align:right;padding:6px;">â‚¹${Math.round(val*g)} </td></tr>`; 
        ...
        if(hist22){ 
            ...
            rows.forEach(r=>{ html+=`<tr style="border-bottom:1px solid #eee;"><td style="padding:6px;">${r.c[0]?.f||''}</td><td style="text-align:right;padding:6px;">â‚¹${r.c[1]?.v||0}</td></tr>`});
        ...
        if(hist24){ 
            ...
            rows.forEach(r=>{ html+=`<tr style="border-bottom:1px solid #eee;"><td style="padding:6px;">${r.c[0]?.f||''}</td><td style="text-align:right;padding:6px;">â‚¹${r.c[3]?.v||0}</td></tr>`});
    ...
    // ====================== FETCH CONFIGS ======================
    fetch('https://aditya-kumar-tech.github.io/mbk/data/gs/silver-groups.json').then(r=>r.json()).then(j=>{silverConfig=j; runSilver(); console.log('âœ… Silver config loaded');});
    fetch('https://aditya-kumar-tech.github.io/mbk/data/gs/gold-groups.json').then(r=>r.json()).then(j=>{goldConfig=j; runGold(); console.log('âœ… Gold config loaded');});
