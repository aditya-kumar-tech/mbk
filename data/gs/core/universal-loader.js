...
window.Silverdata=function(q){
  ...
  /* 🔥 CLEAR GOLD DOM (FULL ISOLATION) */
  ['#g22kt','#g24kt','#gramtbl22','#gramtbl24','#data_table2','#gldgraf']
  .forEach(s=>{const e=has(s); if(e) e.innerHTML='';});
...
function renderSilver(rows){
  ...
  has('#silvr_change')&&(silvr_change.innerHTML=
    `<span class="${d>=0?'up':'down'}">${arrow(d)} ₹${d} (${pct(today,y)}%)</span>`);
...
window.golddata=function(q){
  ...
  /* 🔥 CLEAR SILVER DOM (FULL ISOLATION) */
  ['#silvr_pricet','#silvr_change','#silvr_gramtbl','#data_table1','#silvr_graf']
  .forEach(s=>{const e=has(s); if(e) e.innerHTML='';});
