(function () {
    console.log("🚀 Universal Loader v9.0 – Gold & Silver Safe Loader");
    ...
    function runSilver() {
        ...
        fetch(url)
            ...
            .then(t => {
                ...
                if(window.silvr_pricet || document.getElementById('silvr_gramtbl')) renderSilver(rows);
            })
            .catch(err => { console.error("Silver fetch failed, retrying...", err); setTimeout(runSilver,1500); });
    }
    ...
    function renderSilver(rows) {
        ...
        if(discSilver) discSilver.innerHTML=`<div style="background:#fff3cd;border-left:4px solid #c0c0c0;padding:10px;margin:10px 0;border-radius:5px;font-size:13px;line-height:1.4;">⚠️ Disclaimer: The silver rates are sourced from local jewellers and other sources. Accuracy not guaranteed.</div>`;
    ...
    function runGold() {
        ...
        fetch(url)
            ...
            .then(t => {
                ...
                if(window.g22kt || window.g24kt || document.getElementById('data_table1')) renderGold(rows);
            })
    ...
    function renderGold(rows){
        ...
        if(discGold) discGold.innerHTML=`<div style="background:#fff3cd;border-left:4px solid #f59e0b;padding:10px;margin:10px 0;border-radius:5px;font-size:13px;line-height:1.4;">⚠️ Disclaimer: The gold rates are sourced from local jewellers. Accuracy not guaranteed.</div>`;
    ...
    fetch('https://aditya-kumar-tech.github.io/mbk/data/gs/silver-groups.json')
        .then(r=>r.json()).then(j=>{silverConfig=j; runSilver(); console.log('✅ Silver config loaded');});
    fetch('https://aditya-kumar-tech.github.io/mbk/data/gs/gold-groups.json')
        .then(r=>r.json()).then(j=>{goldConfig=j; runGold(); console.log('✅ Gold config loaded');});
    ...
    window.addEventListener('load', () => {
        ...
        if(window.golddata) golddata("sct180","Gold");
    });
