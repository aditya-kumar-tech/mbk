(function () {
  ...
  function safeVal(v) { return (v === null || v === undefined || v === '' || v === 0 || v === '0') ? '-' : v; }
    ...
    root.innerHTML = `
      <div id="statsSection" style="display: none;">
        <div class="stats" id="stats">
          ...
          <div class="stat-card"><div class="stat-number" id="selectedDate">-</div><div class="stat-label">तारीkh</div></div>
        </div>
      </div>
  ...
  async function loadMandiBhav(mandiId) {
    ...
    try {
      ...
      document.getElementById('mbkApp').style.display = 'block';
    } finally {
    ...
    if (viewMode === 'table') {
      ...
      document.getElementById('tableBody').innerHTML = data.map((r, i) => `
        <tr><td>${i+1}</td>${visibleColumns.map(c => {
          ...
          if(c.idx===3) v = varieties[r[3]] || r[3];
      ...
      el.cardsContainer.innerHTML = data.map((r, i) => `
        <div class="card">
          ...
          <div class="card-grid">
            ${isValid(r[3]) ? `<div class="card-field"><div class="card-label">वैरायटी</div><div class="card-value">${varieties[r[3]]||r[3]}</div></div>`:''}
            ${isValid(r[4]) ? `<div class="card-field"><div class="card-label">ग्रेड</div><div class="card-value">${grades[r[4]]||r[4]}</div></div>`:''}
          </div>
          ...
          ${isValid(r[8]) ? `<div class="card-grid"><div class="card-field"><div class="card-label">आवक (क्विंटल)</div><div class="card-value">${r[8]}</div></div></div>`:''}
        </div>`).join('');
  ...
  function updateStats() {
    ...
    ['statsSection', 'infoSearchSection', 'watermark'].forEach(id => document.getElementById(id).style.display = 'block');
  }
  ...
  async function init() {
    ...
    document.getElementById('searchInput').oninput = (e) => {
      ...
      renderContent(mandiData.filter(r => (commodities[r[2]]||r[2]||'').toLowerCase().includes(q)));
    };
