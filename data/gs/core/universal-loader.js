(function(){
    console.log("🚀 Universal Loader v12.0 - Conditional Chart.js Load");
    ...
    // ====================== Chart Loader ======================
    function loadChartIfNeeded(cb){
        if(window.Chart) return cb();
        const el = document.querySelector('#silvr_graf, #gldgraf');
        if(!el) return; // skip if no chart container
    ...
    if(document.querySelector('#silvr_pricet') || document.querySelector('#silvr_gramtbl')){
        ...
        window.Silverdata=function(q,mtype){
            ...
            if(!document.querySelector('#silvr_pricet')) return;
        ...
        function renderSilver(rows){
            ...
            if(gramTbl){
                ...
                [1,10,50,100,500,1000].forEach(g=>{
                    const val=Math.round((g/1000)*(rows[0].c[2]?.v||0));
    ...
    if(document.querySelector('#g22kt') || document.querySelector('#g24kt')){
        ...
        window.golddata=function(q,mtype){
            ...
            if(!document.querySelector('#g22kt') && !document.querySelector('#g24kt')) return;
        ...
        function renderGold(rows){
            ...
            function updateGram(id,val){
                ...
                if(el){
                    let html='<table style="width:100%;border-collapse:collapse;">';
                    [1,5,10,50,100].forEach(g=>html+=`<tr><td>${g}g</td><td style="text-align:right;">${rs(Math.round(val*g))}</td></tr>`);
                    html+='</table>'; el.innerHTML=html;
                }
            }
            updateGram('gramtbl22',rows[0].c[1]?.v||0); updateGram('gramtbl24',rows[0].c[3]?.v||0);
            ...
            if(hist22){
                ...
                rows.forEach(r=>html+=`<tr><td>${r.c[0]?.f||''}</td><td style="text-align:right;">${rs(r.c[1]?.v||0)}</td></tr>`); html+='</table>'; hist22.innerHTML=html;
            }
            if(hist24){
                let html='<table style="width:100%;border-collapse:collapse;"><tr style="background:#f3e8ff;"><th>Date</th><th>24K Price</th></tr>';
                rows.forEach(r=>html+=`<tr><td>${r.c[0]?.f||''}</td><td style="text-align:right;">${rs(r.c[3]?.v||0)}</td></tr>`); html+='</table>'; hist24.innerHTML=html;
            }
